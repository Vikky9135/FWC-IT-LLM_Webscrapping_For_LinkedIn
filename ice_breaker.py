...
if __name__ == "__main__":
    ...
    # Template to summarize the information and provide two interesting facts
    summary_template = """
    ...
    # Create a prompt template with the input variable
    summary_prompt_template = PromptTemplate(
        input_variables=["information"], template=summary_template
    )

    # Initialize the OpenAI model via LangChain's ChatOpenAI
    llm = ChatOpenAI(temperature=0, model_name="gpt-3.5-turbo")

    # Create a chain of prompt and LLM
    chain = summary_prompt_template | llm

    # Invoke the chain with the information
    res = chain.invoke(input={"information": information})

    # Print the result (summary and interesting facts)
    print(res)
